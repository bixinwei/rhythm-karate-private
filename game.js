...
function createImpact(kind) {
  ...
  const fx = {
    ...
    totalLife: kind === 'perfect' ? 0.45 : TOUCH_FX_SECONDS
  };
...
// TimingAccuracyDisplay plays Just00 for a perfect hit.  Its orbitalZ curve
// is the source's angular orbit around the system origin; radial is the
// independent outward velocity.  Just01 is the separate OK-rating object and
// must never be mixed into the perfect effect.
...
function createHeavenJustParticles() {
  ...
  // TimingAccuracyDisplay.MakeAccuracyVfx() calls Play() on Just00 only.
  // These are its exact particle count/lifetime/speed/shape curve values.
  ...
  for (const fx of touchFx) {
    ...
    if (fx.kind === 'perfect') {
      ...
      for (const star of (fx.heavenParticles?.stars ?? [])) {
        ...
        if (age < star.life) {
          // AceStarParticle's shader offsets every particle's grayscale seed
          // by (_Speed / 4) * _Time, preserving its relative palette position.
...
const PORTED_ASSET_REV = 'gba-ports-hd-22';
...
// 高清派生素材: Spaceball ships an EPX-upscaled (4x) atlas and background, so every
...
function beginStage(ctx, scale) { ctx.setTransform(scale, 0, 0, scale, 0, 0); }function drawPortedCell(cell, x, y, scale = 4, rotation = 0, flipX = false) {
  ...
  if (Number.isFinite(meta.atlasX)) ctx.drawImage(image, meta.atlasX * assetScale, meta.atlasY * assetScale, meta.width * assetScale, meta.height * assetScale, -meta.originX*scale, -meta.originY*scale, meta.width*scale, meta.height*scale);
  else ctx.drawImage(image, 0, 0, image.naturalWidth, image.naturalHeight, -meta.originX * scale, -meta.originY * scale, image.naturalWidth / assetScale * scale, image.naturalHeight / assetScale * scale);
  ctx.restore();
}
